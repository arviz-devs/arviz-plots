...
from arviz_plots import plot_dist, plot_forest, plot_mcse, plot_ridge
...
mcse_rug = st.booleans()
mcse_extra_methods = st.booleans()


@st.composite
def mcse_n_points(draw):
...
@given(
    ...
    rug=mcse_rug,
    n_points=mcse_n_points(),
    extra_methods=mcse_extra_methods,
)
def test_plot_mcse(datatree, rug, n_points, extra_methods, plot_kwargs):
    pc = plot_mcse(
        datatree,
        backend="none",
        rug=rug,
